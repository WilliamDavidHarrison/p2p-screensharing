...
<template>
    <section class="h-full w-full flex flex-col">
        <div class="hidden lg:flex bg-slate-600 h-12 items-center px-3 shadow">
            <h1 class="font-semibold">Chat (<span class="text-blue-400">{{ username }}</span>)</h1>
        </div>

        <ul ref="chatEl" id="chat-container" class="overflow-y-scroll flex flex-col-reverse p-2">
            <li v-for="message in messages" :key="message.id" class="mb-2">
                <strong
                    ...
                    ><span v-if="message.username === username">You</span><span v-else>{{ message.username }}</span>:</strong>
                {{ message.text }}
            </li>
        </ul>
