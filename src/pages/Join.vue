...
onMounted(() => {
    peer.on("error", (e: unknown) => {
        ...
        if(err.type === "peer-unavailable") invalidId.value = true;
    });
});
...
<template>
    ...
    <Modal :show="username.length > 0 && !connectionToPeer && !invalidId && !rejected">
        <WaitingModal title="Requesting to join room..." @cancel="router.push('/')" />
    </Modal>
